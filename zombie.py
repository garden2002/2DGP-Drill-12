...
class Zombie:
    ...
    def run_slightly_to(self, tx, ty):
        self.dir = math.atan2(ty -self.y ,tx - self.x) + math.pi
        distance = RUN_SPEED_PPS * game_framework.frame_time
        self.x += distance * math.cos(self.dir)
        self.y += distance * math.sin(self.dir)
        self.x = clamp(25, self.x, 1280 - 25)
        self.y = clamp(50, self.y, 1024 - 40)
        pass
    ...
    def build_behavior_tree(self):
        ...
        wander = Sequence('Wander', a2, a1)

        c1 = Condition('소년이 근처에 있는가?',self.is_boy_nearby,7)
        c2 = Condition('공이 더 많은가?', self.is_many_balls)

        a3 = Action('소년에게 접근' , self.move_to_boy)
        chase_boy = Sequence('소년을 추적',c2 ,a3)

        a4 = Action('소년에게 도망' , self.run_to_boy)
        run_boy = Sequence('좀비 도망',a4)

        s1 = Selector('추적 또는 도망 선택',chase_boy,run_boy)
        chase_or_runaway = Sequence('추적 또는 도망', c1 , s1)

        root = chase_or_flee = Selector('추적 또는 배회',chase_or_runaway,wander)
